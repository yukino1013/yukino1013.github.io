...
1. **「应用管理一页 12 个位置」怎么理解？**
   需求说明书原文是「应用管理界面一页由 3 行 4 列，共 12 个应用位置组成」。
   当前实现是 **4 列网格 + 一页容量 12，有几张卡渲染几张**（装了 5 个应用就显示 5 格）。
   另一种理解是**恒定显示 12 个格子**（空位也画出来）。
   若用户选后者，改 `assets/js/vod-demo.js` 的 `render()` 里 `v-grid` 的铺法即可；
   注意 `test_vod.js` 里有两项断言（2.2.1、2.2.2）与当前铺法绑定，需要同步调整。

2. **头像用的是动漫角色插画**（`assets/img/avatar.webp`，Persona 系列风格，导航栏与名片共用）。
   这是用户明确指定要换的图，但求职场景下动漫头像可能影响专业观感，已提示、用户尚未回应。
   若要换：直接替换 `assets/img/avatar.webp` 即可，建议正方形、256×256 以上。
